...
```{r}
x <- 2.45 # Crio a variavel
x <- as.integer(x) # Transformo em inteiro
class(x) # Verifico a classe
```
...
m1 <- matrix(c(1,3,5,2,5,6,3,2,4),nc = 3,nr = 3)
m1
...
m2 <- matrix(c(13,173515,246,27,1,87,321,3857,4229),nc = 3,nr = 3)
m2
...
```{r}
df1 <- data.frame(
  Continente = c("America","Europa","Asia","Africa"),
  Paises = c("Brasil","Portugal","China","Togo"),
  IDH = c(0.5,0.9,0.6,0.4)
)
...
df <- data.frame(Coluna1 = c(1,2),
                 Coluna2 = c(3,4))

m1 <- matrix(seq(1,8),nc = 2)
